...
import { Loader2, Search, Eye, Clock, Wrench, Package } from 'lucide-react';
...
export default function TechnicianAssignedPage() {
  ...
  const fetchAssignedRepairs = async () => {
    try {
      ...
      if (!res.ok) throw new Error('Failed to fetch assigned repairs');
      const data = await res.json();
      setRepairs((data.repairs || []).filter((r) => ACTIVE_STATUSES.includes(r.repairStatus)));
    } catch (error) {
      console.error(error);
    } finally {
      setLoading(false);
  ...
  const statusBadge = (status) => {
    if (status === 'pending') return 'bg-amber-500/20 border-amber-500/30 text-amber-400';
    if (status === 'in-progress') return 'bg-blue-500/20 border-blue-500/30 text-blue-400';
    if (status === 'waiting-parts') return 'bg-orange-500/20 border-orange-500/30 text-orange-400';
  ...
  return (
    <div className="flex h-screen bg-[#0A0F1E]">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <TopBar title="Assigned Jobs" breadcrumbs={['Technician', 'Assigned Jobs']} />

        <div className="flex-1 overflow-auto p-6 space-y-5">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="glass rounded-xl p-4 border border-white/10"><p className="text-gray-400 text-sm">Total Active Jobs</p><p className="text-2xl font-bold text-white mt-1">{repairs.length}</p></div>
              ...
              <table className="w-full">
                <thead className="bg-white/5">
                  <tr>
                    ...
                    <th className="text-left py-3 px-5 text-gray-400 text-sm font-medium">Issue</th>
                ...
                <tbody>
                  {filtered.length === 0 ? (
                    <tr><td colSpan="6" className="text-center py-10 text-gray-500">No assigned active jobs found</td></tr>
                  ) : filtered.map((r) => (
                    <tr key={r._id} className="border-t border-white/5 hover:bg-white/5 transition-all">
                      ...
                      <td className="py-3 px-5 text-gray-300 text-sm max-w-[280px] truncate">{r.problemDescription}</td>
                      <td className="py-3 px-5">
                        <span className={`px-2 py-0.5 rounded text-xs font-medium border ${statusBadge(r.repairStatus)}`}>
