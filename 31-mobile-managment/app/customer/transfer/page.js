...
import {
  ArrowLeftRight, Smartphone, CheckCircle, XCircle,
...
export default function CustomerTransferPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/customer/transfer?customerId=${customerId}`)
      .then(res => res.json())
  ...
  const fetchTransfers = async () => {
    try {
      setLoading(true);
      const res = await fetch(`/api/customer/transfer?customerId=${user.id || user._id}`);
  ...
  const handleAction = async (transferId, action) => {
    ...
    try {
      const res = await fetch('/api/customer/transfer', {
        ...
        body: JSON.stringify({ transferId, customerId: user.id || user._id, action }),
      });
      const data = await res.json();
        ...
        <div className="flex-1 overflow-auto p-6 space-y-5">
          ...
          {pendingCount > 0 && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
              className="p-4 bg-purple-600/10 border border-purple-500/30 rounded-xl flex items-center gap-3">
            ...
            <div className="space-y-4">
              {filtered.map((t, i) => (
                <motion.div key={t._id}
                  ...
                  <div className="flex items-start justify-between gap-3 mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        {t.device?.color && <p className="text-gray-500 text-xs">{t.device.color} · {t.device.storage}</p>}
                      </div>
                    </div>
                  ...
                  {t.transferNote && (
                    <p className="text-gray-400 text-sm mb-4 italic">"{t.transferNote}"</p>
                  )}
                  ...
                  {t.status === 'completed' && (
                    <p className="text-green-400 text-sm flex items-center gap-1.5">
                      <CheckCircle className="w-4 h-4" />
                      Accepted on {new Date(t.completedAt).toLocaleDateString()} — device is now yours
                    </p>
                  )}
                  {t.status === 'rejected' && (
                    <p className="text-red-400 text-sm flex items-center gap-1.5">
                      <XCircle className="w-4 h-4" />
                      Rejected on {new Date(t.rejectedAt).toLocaleDateString()}
                    </p>
                  )}
                </motion.div>
              ))}
